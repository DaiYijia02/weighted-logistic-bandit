...
def fit_online_logistic_estimate(context, reward, current_estimate, vtilde_matrix, vtilde_inv_matrix, constraint_set_radius,
    ...
    inv_z_arm = np.dot(sqrt_vtilde_inv_matrix, context)
    ...
    for _ in range(iters):
        ...
        z_estimate = project_ellipsoid(x_to_proj=unprojected_update,
                                       ell_center=np.zeros_like(context),
                                       ecc_matrix=vtilde_matrix,
                                       radius=constraint_set_radius)
    theta_estimate = np.dot(sqrt_vtilde_inv_matrix, z_estimate)
    return theta_estimate

def fit_online_logistic_estimate_ranking(context, reward, ranking_weight, current_estimate, vtilde_matrix, vtilde_inv_matrix, constraint_set_radius,
                                 diameter=1, precision=0.1):
    """
    ECOLog estimation procedure.
    """
    ...
    # few steps of projected gradient descent
    for _ in range(iters):
        pred_probas = ranking_weight * sigmoid(np.sum(z_estimate * inv_z_arm))
        grad = z_estimate - z_theta_t + (pred_probas - reward) * inv_z_arm
        unprojected_update = z_estimate - step_size * grad
        z_estimate = project_ellipsoid(x_to_proj=unprojected_update,
                                       ell_center=np.zeros_like(context),
...
def fit_online_logistic_estimate_bar(context, current_estimate, vtilde_matrix, vtilde_inv_matrix, constraint_set_radius,
                                     diameter=1, precision=0.1):
    """
...
def fit_online_logistic_estimate_bar_ranking(context, current_estimate, ranking_weight, vtilde_matrix, vtilde_inv_matrix, constraint_set_radius,
                                     diameter=1, precision=0.1):
    """
    ECOLog estimation procedure to compute theta_bar.
    """
